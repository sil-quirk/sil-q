...
| 4 | RNG + math helpers | **Done** | Replaced `z-rand.c/z-rand.h` with `src/rng.c/rng.h`; exact algorithm preservation ensures save/load compatibility; logged in `session_notes.md:3-110`. |
...
| `z-rand.c/h` | **RETIRED** - Replaced with `src/rng.c/rng.h` in Phase 4. | New module preserves exact algorithms and API for 100% compatibility; positioned for future SDL3 RNG integration. |
...
**Scope:** Replace legacy RNG with modern module while preserving exact gameplay behavior.
- Created `src/rng.c` and `src/rng.h` with modern documentation and C17 types (`bool`, `size_t`).
- Preserved exact algorithms from `z-rand.c` to ensure deterministic gameplay and save/load compatibility.
- Maintained all API compatibility: `rand_int()`, `dieroll()`, `rand_range()`, `one_in_()`, `percent_chance()`, etc.
- Kept RNG state variables (`Rand_quick`, `Rand_value`, `Rand_place`, `Rand_state[]`) for save format compatibility.
- Deleted `z-rand.c` (590 lines) and `z-rand.h` (91 lines).
- Updated `CMakeLists.txt` and `angband.h` to use new module.
- Clean build with no new warnings; all ~500+ call sites work unchanged.
- Foundation ready for future SDL3 `SDL_RandomContext` migration.
...
5. **z-form/z-rand/z-term retirement:**
   * ~~**z-rand** - COMPLETED (Phase 4): Created `src/rng.c` and `src/rng.h` to replace `z-rand.c/z-rand.h`. Files deleted, functionality migrated with 100% API compatibility.~~
   * **z-form** - Planned: Create `src/format.c` to replace formatting helpers.
   * **z-term** - Planned: Create `src/ui/term_sdl.c` and migrate callers incrementally.
   * After each migration, remove the corresponding chunk from the old `z-*` file so we can delete the file instead of leaving dead code behind.
6. **Header hygiene:**
   * ~~**angband.h** - COMPLETED (Phase 4): Changed from `#include "z-rand.h"` to `#include "rng.h"`.~~
   * Stop including remaining `z-*.h` from `angband.h` once functionality moves. Replace macros with `static inline` functions or typed enums to better leverage compiler diagnostics.

## C17 Modernization Checklist

* ~~**z-rand.c/z-rand.h** - COMPLETED (Phase 4): Replaced with `src/rng.c/rng.h` using modern C17 types (`bool` from `<stdbool.h>`), clear documentation, and foundation for future SDL3 integration.~~
