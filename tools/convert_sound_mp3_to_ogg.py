#!/usr/bin/env python3
"""Convert MP3 audio under lib/xtra/sound to sibling OGG files.

By default this scans the repository's ``lib/xtra/sound`` tree and writes
``.ogg`` files next to each ``.mp3``.

Examples:
    python tools/convert_sound_mp3_to_ogg.py
    python tools/convert_sound_mp3_to_ogg.py --dry-run
    python tools/convert_sound_mp3_to_ogg.py --delete-source --overwrite
"""
...
DEFAULT_ROOT = REPO_ROOT / "lib" / "xtra" / "sound"
DEFAULT_QUALITY = "5"
OGG_CODEC = "libvorbis"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert .mp3 files under lib/xtra/sound to sibling .ogg files."
    )
    parser.add_argument(
        "root",
        nargs="?",
        default=str(DEFAULT_ROOT),
        help="Root directory to scan (defaults to repo lib/xtra/sound).",
    )
    parser.add_argument(
        "--ffmpeg",
        default="ffmpeg",
    ...
    parser.add_argument(
        "--delete-source",
        action="store_true",
        help="Delete the source .mp3 after a successful conversion.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> int:
    ...
    for source in iter_mp3_files(root):
        destination = source.with_suffix(".ogg")
        ...
        if args.delete_source:
            try:
                ...
                failed += 1
                print(
                    f"[error] {rel_source}: could not delete source: {exc}",
                    file=sys.stderr,
                )
            else:
    ...
    if args.dry_run:
        print(
            f"Would convert {would_convert} file(s); skipped {skipped} existing "
            f"output(s)."
        )
    else:
        print(
