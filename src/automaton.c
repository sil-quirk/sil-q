 ...
 *
 * The idea and some of the code from five of the low-level utility functions
 * came from the Angband Borg by Ben Harrison (with modifications by Dr Andrew
 * White).
 *
 * The borrowed functions are: automaton_keypress, automaton_keypresses,
 * automaton_inkey, automaton_flush, automaton_inkey_hack
 *
 * Inspiration to actually start writing an AI for Sil came from Brian Walker's
 * article: 'The Incredible Power of Djikstra Maps'.
 *
 * The Angband Borg was written with the idea that it would interact with the
 * game as if it were a player at a terminal. Thus, it would have to scan the
 * screen to get information about what was going on and then input characters
 * to control the game, making it the full AI experience with no assistance from
 * the game. When it got assistance on certain occasions, this was seen as a
 * crutch and a 'cheat' of sorts.
 *
 * I don't intend to be that masochistic. While it would be nice, the basic
 * parts of the AI that are needed (a *lot* of screen-scraping), are not very
 * interesting. I'm more concerned with the AI needed to play Sil well as
 * opposed to working out what the letters on the screen mean. I'm thus prepared
 * to let the AI have access to internal variables when any sensible player
 * would know their content, and even to modify the main program a bit to add
 * flags which make it easier for the AI to know what is going on.
 *
 * However, I shall try to follow a rule that it shouldn't have access
 * to info that the player doesn't have (such as what items do before
 * identified) and it shouldn't be able to perform actions that the player
 * can't. At some points it might break these rules, but I'll flag that in the
 * code as 'cheating' and try to remove it later.
 *
 *
 * The basic approach is that when someone activates the automaton, it replaces
 * the standard function that gathers user keypresses. Thus, whenever the
 * program awaits input, the automaton can supply it. The automaton works out
 * what keypresses to send, and queues one or more of them up in the
 * 'automaton_key_queue'. These are then consumed by the standard game, causing
 * it to run around etc. Real user keypresses abort the automaton and return
 * control to the user.
 *
 * There is a small amount of automaton-related code in other files, but the aim
 * is to have almost all of it here.
 */
...
/*
 * An array storing the automaton's additional internal info about the dungeon
 * layout
 *
 * Currently this is just used as a boolean array for storing whether the
 * contents of the square are known (necessary because unlit floor squares can't
 * get set with CAVE_MARK).
 *
 * It could be expanded so that each square has multiple pieces of information.
 * This could be done with bitflags (like cave_info), or even better might be an
 * map_square struct that can store numbers, indices for monsters etc. If it is
 * too hard to get a 2D array of structs to work (I couldn't do it...) then
 * multiple 2D arrays might be best.
 */
...
errr automaton_keypress(char k)
{
    ...
    if (automaton_key_head == KEY_SIZE)
        automaton_key_head = 0;

    /* Hack -- Catch overflow (forget oldest) */
    if (automaton_key_head == automaton_key_tail)
        return (-1);

    /* Hack -- Overflow may induce circular queue */
    if (automaton_key_tail == KEY_SIZE)
        automaton_key_tail = 0;
...
/*
 * Updates an array the size of the map with information about how long the
 * automaton thinks it will take the player to get to the given centre square
 * from any map square.
 *
 * The code is heavily based on update_flow() from cave.c, so see there for full
 * comments.
 *
 * This is separated in an attempt to keep as much automaton stuff as possible
 * out of the main game files (and in the hope that the automaton flow code can
 * be tailored in future).
 */
        ...
        for (i = 0; i < last_index; i++)
        {
            ...
            else
            {
                ...
                for (d = 0; d < 8; d++)
                {
                    ...
                    // skip chasms
                    if (cave_feat[y2][x2] == FEAT_CHASM)
                        continue;

                    // penalise traps
                    if (cave_trap_bold(y2, x2)
                        && !(cave_info[y2][x2] & (CAVE_HIDDEN)))
                        extra_cost += 3;
                    ...
                    if (cave_m_idx[y2][x2] > 0)
                    {
                        ...
                        if (n_ptr->ml && q_ptr->flags1 & (RF1_NEVER_MOVE))
                            extra_cost += 10;
                    }
...
bool find_enemy_to_kill(int* ty, int* tx)
{
    ...
    for (i = 1; i < mon_max; i++)
    {
        ...
        if (dist < best_dist)
        {
            best_dist = dist;
            can_fire
                = (dist > 1) && (cave_info[m_ptr->fy][m_ptr->fx] & (CAVE_FIRE));
    ...
    if (can_fire && inventory[INVEN_BOW].tval
        && ((inventory[INVEN_QUIVER1].number >= 1)
            || (inventory[INVEN_QUIVER2].number >= 1)))
    {
        // clear the target
        target_set_monster(0);

        // queue the commands
        automaton_keypresses("ff"); // bug: occasionally shoots at a mold
                                    // instead of the mobile target...
...
int evaluate_weapon(object_type* o_ptr)
{
    ...
    value
        += (o_ptr->att * skill_vals[S_MEL]) + (o_ptr->evn * skill_vals[S_EVN]);
...
int evaluate_bow(object_type* o_ptr)
{
    ...
    // value skill points as per normal
    value
        += (o_ptr->att * skill_vals[S_ARC]) + (o_ptr->evn * skill_vals[S_EVN]);
...
int evaluate_object(object_type* o_ptr)
{
    ...
    if (slot >= 0)
    {
        ...
        case INVEN_QUIVER2:
            ...
            // by counting those in pack and considering how much we care about
            // archery
...
bool pickup_object(void)
{
    ...
    if (value > 0)
    {
        ...
        {
            // if it is considered equal in value to existing arrows, then just
            // get it to allow auto-merging
            if (value == 1)
            {
                // create the commands
                strnfmt(commands, sizeof(commands), "g-");
            ...
            // otherwise it is considered better than the existing arrows so
            // wield it...
            else
            {
                // create the commands
                ...
                // if both slots are full, we need to tell the game to replace
                // the inferior one
                if (inventory[INVEN_QUIVER1].k_idx
                    && inventory[INVEN_QUIVER2].k_idx)
                {
                    if (evaluate_arrow(&inventory[INVEN_QUIVER1])
                        > evaluate_arrow(&inventory[INVEN_QUIVER2]))
                    {
...
{
    if ((p_ptr->chp * 100 / p_ptr->mhp < 75) || (p_ptr->stun)
        || (p_ptr->confused) || (p_ptr->afraid) || (p_ptr->poisoned)
...
void find_unexplored(int* ty, int* tx)
{
    ...
    // first, if you are in a suspicious dead-end corridor, possibly search a
    // bit
            ...
            for (x = 1; x < p_ptr->cur_map_wid - 1; x++)
            {
                if (!((cave_info[y][x] & (CAVE_MARK)) || automaton_map[y][x]))
                {
                    ...
                    for (i = 7; i >= 0; i--)
                    {
                        ...
                        // keep track of the best square adjacent to the
                        // unmarked location that is reachable from the player
                        if (dist < best_local_dist)
                            best_local_dist = dist;
                    }

                    // keep track of the closest unmarked location
                    // (the second line breaks ties in a way that makes it go
                    // around corridor corners properly)
                    if ((best_local_dist < best_dist)
                        || ((best_local_dist == best_dist)
                            && (best_local_dist == 1)
 ...
 *
 * Exploring
 *  - be less completist with levels (don't walk 50 squares to explore one more
 * room...)
 ...
 *
 *
 * In general, I wrote most of the basic routine as an AI with no internal
 * state. I've added state in the form of its internal map and more state should
 * be added (for example so it can work out if it lost health since last turn,
 * or to set its own internal mode).
 */
...
void automaton_turn(void)
{
    ...
    // allocate experience
    if (ty == 0)
        if (allocate_experience())
            return;

    // otherwise: fight any nearby alert monsters
    if (ty == 0)
        if (find_enemy_to_kill(&ty, &tx))
            return;
    ...
    // otherwise: find the closest unexplored location that is next to an
    // explored one
    if (ty == 0)
        find_unexplored(&ty, &tx);
    ...
    else
    {
        ...
        // work out the adjacent square closest to the target (with preference
        // for orthogonals)
        for (i = 7; i >= 0; i--)
        {
            // get the location
...
char automaton_inkey_hack(int flush_first)
{
    ...
    if (flush_first)
    {
        ...
        {
            // Flush keys
            ////automaton_flush();   currently not actually doing this as it
            ///stops us queuing a 'y' for stepping on traps
        }
    }
    ...
    if (!i)
    {
        ...
        {
            // takes its turn by choosing some keys representing commands and
            // queuing them
            automaton_turn();
