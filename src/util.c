...
/*
 *=============================================================================
 * SDL-based file I/O operations
 * These functions use SDL3's SDL_IOStream API for cross-platform file access
 *=============================================================================
 */
 ...
 * Returns SDL_IOStream* instead of FILE*
 */
SDL_IOStream* sdl_fopen(cptr file, cptr mode)
{
    ...
    /* Attempt to open the file using SDL */
    stream = SDL_IOFromFile(buf, mode);

    if (!stream)
    {
        log_debug("sdl_fopen: Failed to open '%s' with mode '%s': %s", buf, mode, SDL_GetError());
    }
...
/*
 * SDL-based replacement for "fclose()"
 */
...
/*
 * SDL-based temporary file creation
 */
...
errr sdl_fgets(SDL_IOStream* stream, char* buf, size_t n)
{
    ...
    while (i < len)
    {
        ...
        /* Read next character */
        bytes_read = SDL_ReadIO(stream, &c, 1);
        ...
        /* Handle carriage return (for DOS/Windows text files) */
        if (c == '\r')
        {
            /* Peek ahead for \n */
            unsigned char next;
            ...
            if (SDL_ReadIO(stream, &next, 1) == 1)
            {
                if (next == '\n')
                {
                    /* CR+LF pair - treat as end of line */
                    *s = '\0';
                    return (0);
                }
                else
                {
                    /* Just CR - seek back and continue */
                    SDL_SeekIO(stream, pos, SDL_IO_SEEK_SET);
                }
            }
            /* Continue processing the CR as a regular character if needed */
        }
...
/*
 * SDL-based replacement for "fputs()"
 * 
 * Dump a string, plus a newline, to a stream
 */
errr sdl_fputs(SDL_IOStream* stream, cptr buf, size_t n)
{
    ...
    /* Use SDL_IOprintf for formatted output */
...
errr sdl_read(SDL_IOStream* stream, char* buf, size_t n)
{
    ...
    /* Read the data */
    bytes_read = SDL_ReadIO(stream, buf, n);
...
/*
 * SDL-based file write operation
 * Writes exactly n bytes to the stream
 */
errr sdl_write(SDL_IOStream* stream, cptr buf, size_t n)
{
    ...
    /* Write the data */
    bytes_written = SDL_WriteIO(stream, buf, n);
...
errr sdl_seek(SDL_IOStream* stream, Sint64 offset)
{
    ...
    result = SDL_SeekIO(stream, offset, SDL_IO_SEEK_SET);
    ...
    /* Failure - didn't reach desired position */
    if (result != offset)
    {
...
/*
 * SDL-based file size operation
 * Returns size of stream, or -1 on error
 */
